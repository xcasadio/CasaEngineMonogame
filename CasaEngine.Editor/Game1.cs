...
namespace CasaEngine.Editor
{
    public class Game1 : Game, IObservableUpdate
    {
        private sealed class EditorRawInputSource : IRawInputSource
        {
            ...
            {
                var fallbackState = Microsoft.Xna.Framework.Input.Mouse.GetState();
                return GetWindowMouseState(fallbackState);
            }
            ...
            private MouseState GetWindowMouseState(MouseState fallbackState)
            {
                ...
                LogMouseTransition("EditorRawInput", fallbackState, result);
                return result;
            }

            private void LogMouseTransition(string source, MouseState fallbackState, MouseState currentState)
            {
                var previousState = _lastLoggedMouseState;
                if (previousState.HasValue
                    && previousState.Value.LeftButton == currentState.LeftButton
                    && previousState.Value.MiddleButton == currentState.MiddleButton
                    && previousState.Value.RightButton == currentState.RightButton
                    && previousState.Value.X == currentState.X
                    && previousState.Value.Y == currentState.Y)
                {
                    return;
                }
                ...
                {
                    Debug.WriteLine(
                        $"[{source}] pos=({currentState.X},{currentState.Y}) l={currentState.LeftButton} m={currentState.MiddleButton} r={currentState.RightButton} " +
                        $"fallback=({fallbackState.X},{fallbackState.Y})/{fallbackState.LeftButton}");
                }
