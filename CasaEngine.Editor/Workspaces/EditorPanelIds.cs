namespace CasaEngine.Editor.Workspaces;

public static class EditorPanelIds
{
    public const string WorldViewport = "panel_world_viewport";
    public const string ContentBrowser = "panel_content_browser";
    public const string Output = "panel_output";
    public const string Entities = "panel_entities";
    public const string EntityDetails = "panel_entity_details";
    public const string UIScreenHierarchy = "panel_ui_screen_hierarchy";
    public const string UIScreenInspector = "panel_ui_screen_inspector";
    public const string UIScreenToolbox = "panel_ui_screen_toolbox";
    public const string UIScreenDocumentPrefix = "panel_ui_screen_";
}