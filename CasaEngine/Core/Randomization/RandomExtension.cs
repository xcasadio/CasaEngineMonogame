...
namespace CasaEngine.Core.Randomization;
...
public static class RandomExtension
{
    ...
    public static int NextInt32(this Random random, int min, int max) => System.Math.Abs(random.Next() % (max - min + 1)) + min;
    ...
    public static long NextLong(this Random random, long min, long max)
    {
        byte[] buffer = new byte[8];
        random.NextBytes(buffer);
        return System.Math.Abs(BitConverter.ToInt64(buffer, 0) % (max - min + 1L)) + min;
    }
