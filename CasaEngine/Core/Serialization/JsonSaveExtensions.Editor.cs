...
public static class JsonSaveExtensions
{
    ...
    public static void Save(this Coordinates coordinates, JObject jObject)
    {
        ...
        coordinates.Position.Save(positionNode);
        jObject.Add("position", positionNode);

        var scaleNode = new JObject();
        coordinates.Scale.Save(scaleNode);
        jObject.Add("scale", scaleNode);

        var rotationNode = new JObject();
        coordinates.Orientation.Save(rotationNode);
        jObject.Add("rotation", rotationNode);
    }
