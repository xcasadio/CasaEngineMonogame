...
namespace CasaEngine.Framework.GUI.NoesisGUIWrapper.Helpers.DeviceState
{
    ...
    internal class DeviceStateHelperD3D11 : DeviceStateHelper
    {
        ...
        private RawViewportF[] viewports = Array.Empty<RawViewportF>();
        ...
        protected override void Restore()
        {
            ...
            context.OutputMerger.SetBlendState(blendState, blendFactor, sampleMaskRef);
            blendState?.Dispose();

            context.OutputMerger.SetDepthStencilState(depthState, stencilRefRef);
            depthState?.Dispose();

            context.OutputMerger.SetRenderTargets(depthStencilView, renderTargetView[0]);
            depthStencilView?.Dispose();
            renderTargetView[0]?.Dispose();

            context.PixelShader.Set(ps);
            context.PixelShader.SetConstantBuffers(0, psConstantBuffers);
            context.PixelShader.SetSamplers(0, psSamplers);
            context.PixelShader.SetShaderResources(0, psResources);
            ps?.Dispose();
            DisposeArray(psConstantBuffers);
            DisposeArray(psSamplers);
            DisposeArray(psResources);

            context.VertexShader.Set(vs);
            context.VertexShader.SetConstantBuffers(0, vsConstantBuffers);
            context.VertexShader.SetSamplers(0, vsSamplers);
            context.VertexShader.SetShaderResources(0, vsResources);
            vs?.Dispose();
            DisposeArray(vsConstantBuffers);
            DisposeArray(vsSamplers);
            DisposeArray(vsResources);

            context.InputAssembler.SetIndexBuffer(ib, ibFormat, ibOffset);
            ib?.Dispose();

            context.InputAssembler.SetVertexBuffers(0, vb, vbStride, vbOffset);
            DisposeArray(vb);
        }
        ...
        {
            var context = device.ImmediateContext;
            topology = context.InputAssembler.PrimitiveTopology;
            layout = context.InputAssembler.InputLayout;

            var rasterizer = context.Rasterizer;
            SaveViewports(rasterizer);
            //this.scissorRectangles = rasterizer.GetScissorRectangles<RawRectangle>();
            rasterizerState = rasterizer.State;
            blendState = context.OutputMerger.GetBlendState(out blendFactor, out sampleMaskRef);
            depthState = context.OutputMerger.GetDepthStencilState(out stencilRefRef);
            renderTargetView = context.OutputMerger.GetRenderTargets(1, out depthStencilView);

            var pixelShaderStage = context.PixelShader;
            ps = pixelShaderStage.Get();
            ShaderStageGetConstantBuffers(pixelShaderStage, 0, 4, psConstantBuffers);
            ShaderStageGetSamplers(pixelShaderStage, 0, 4, psSamplers);
            ShaderStageGetResources(pixelShaderStage, 0, 4, psResources);

            var vertexShaderStage = context.VertexShader;
            vs = vertexShaderStage.Get();
            ShaderStageGetConstantBuffers(vertexShaderStage, 0, 4, vsConstantBuffers);
            ShaderStageGetSamplers(vertexShaderStage, 0, 4, vsSamplers);
            ShaderStageGetResources(vertexShaderStage, 0, 4, vsResources);

            context.InputAssembler.GetIndexBuffer(out ib, out ibFormat, out ibOffset);
            context.InputAssembler.GetVertexBuffers(0, 1, vb, vbStride, vbOffset);
        }
        ...
        private void SaveViewports(RasterizerStage rasterizer)
        {
            ...
            {
                viewports = new RawViewportF[count];
            }

            rasterizer.GetViewports(viewports);
        }
    }
}