...
public class OctreeNode<T>
{
    ...
    public OctreeNode<T> MoveContainedItem(OctreeItem<T> item, BoundingBox newBounds)
    {
        ...
        else
        {
            ...
            if (item.Container == null)
            {
                ...
                System.Diagnostics.Debug.Assert(node == GetRootNode());
    ...
    private bool CoreAddItem(OctreeItem<T> item)
    {
        ...
        if (_items.Count >= MaxChildren && Children.Length == 0)
        {
            ...
            if (newNode != null)
            {
                ...
                System.Diagnostics.Debug.Assert(succeeded, "Octree node returned from SplitChildren must fit the item given to it.");
                return true;
            }
        }
        ...
        foreach (var child in Children)
        {
            System.Diagnostics.Debug.Assert(child.Bounds.Contains(item.Bounds) != ContainmentType.Contains);
        }
#endif
    ...
    {
        System.Diagnostics.Debug.Assert(Children.Length == 0, "Children must be empty before SplitChildren is called.");
        ...
        for (var x = -1f; x <= 1f; x += 2f)
        {
            for (var y = -1f; y <= 1f; y += 2f)
            {
                for (var z = -1f; z <= 1f; z += 2f)
                {
                    ...
                    {
                        System.Diagnostics.Debug.Assert(childBigEnough == null);
        ...
        {
            System.Diagnostics.Debug.Assert(Children[g] != null);
        }
#endif
    ...
    private void PushItemsToChildren()
    {
        ...
        for (var i = 0; i < _items.Count; i++)
        {
            for (var c = 0; c < Children.Length; c++)
            {
                System.Diagnostics.Debug.Assert(Children[c].Bounds.Contains(_items[i].Bounds) != ContainmentType.Contains);
            }
        }
#endif
    }
    ...
    internal bool TryGetContainedOctreeItem(T item, out OctreeItem<T> octreeItem)
    {
        ...
        {
            var child = Children[i];
            System.Diagnostics.Debug.Assert(child != null, "node child cannot be null.");
    ...
    private class OctreeNodeCache
    {
        ...
        {
            System.Diagnostics.Debug.Assert(!_cachedNodes.Contains(child));
...
#if DEBUG
                System.Diagnostics.Debug.Assert(children.Length == NumChildNodes);
                for (var i = 0; i < children.Length; i++)
                {
                    System.Diagnostics.Debug.Assert(children[i] == null);
                }
#endif
