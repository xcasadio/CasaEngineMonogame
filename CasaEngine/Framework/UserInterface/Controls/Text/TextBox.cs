...
namespace CasaEngine.Framework.UserInterface.Controls.Text
{
    ...
    public class TextBox : ClipControl
    {
        ...
        private void ClientAreaDraw(object sender, DrawEventArgs e)
        {
            ...
            if (Text != null && _font != null)
            {
                ...
                if (_showCursor && _caretVisible)
                {
                    ...
                    if (_mode == TextBoxMode.Multiline)
                    {
                        rc = new Rectangle(r.Left + (int)size.X - _horizontalScrollBar.Value, r.Top + (PositionY - _verticalScrollBar.Value) * _font.LineSpacing, cursor.Width, _font.LineSpacing);
                    }
                    cursor.Alignment = al;
        ...
        private int CharAtPos(Point pos)
        {
            ...
            if (_mode == TextBoxMode.Multiline)
            {
                py = _verticalScrollBar.Value + (y - ClientTop) / _font.LineSpacing;
                if (py < 0)
                {
                    py = 0;
                }
