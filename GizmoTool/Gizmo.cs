...
public class Gizmo : IDisposable
{
    ...
    private BoundingSphere XSphere =>
        new(Vector3.Transform(_translationLineVertices[1].Position, _gizmoWorld),
            RADIUS * _screenScale);

    private BoundingSphere YSphere =>
        new(Vector3.Transform(_translationLineVertices[7].Position, _gizmoWorld),
            RADIUS * _screenScale);

    private BoundingSphere ZSphere =>
        new(Vector3.Transform(_translationLineVertices[13].Position, _gizmoWorld),
            RADIUS * _screenScale);
    ...
    /// <summary>
    /// The viewport used by <see cref="ConvertMouseToRay"/> for Unproject calculations.
    /// Must match the render-target dimensions of the view being edited.
    /// Set this before calling <see cref="Update"/> each frame.
    /// </summary>
            ...
            else if (WasButtonHeld(MouseButtons.Left) && ActiveAxis != GizmoAxis.None)
            {
                ...
                switch (ActiveMode)
                {
                    ...
                    case GizmoMode.Translate:
                    {
                        ...
                        if (SnapEnabled)
                        {
                            var snapValue = TranslationSnapValue;
                    ...
                    case GizmoMode.Rotate:
                    {
                        ...
                        var delta = mousePosition.X - _lastMouseState.X;
    ...
    private Ray ConvertMouseToRay(Vector2 mousePosition)
    {
        ...
        nearPoint = ActiveViewport.Unproject(nearPoint,
                                             _projection,
                                             _view,
                                             Matrix.Identity);
        farPoint = ActiveViewport.Unproject(farPoint,
                                            _projection,
                                            _view,
                                            Matrix.Identity);
