...
| --- | --- | --- |
| [ai-guidelines-tasks.md](tasks/ai-guidelines-tasks.md) | Fichiers de guidage des agents IA : `AGENTS.md` source unique, règles par chemin et agents jumeaux, skills, `settings.json` et hook, ADR (branche `ai-guidelines`) | Phases 0 → 5 ✅ (32 ADR, index et docs corrigés) ; clôture T6.1 🧪 : vérifications manuelles du chargement par l'auteur (Claude Code `/context`, VS Code Copilot), puis merge de la branche (décision humaine). Audit : [analysis-ai-agent-files.md](audits/analysis-ai-agent-files.md). |
