...
| --- | --- | --- |
| [asset-handles-migration-tasks.md](tasks/asset-handles-migration-tasks.md) | Migrer tous les appelants vers les handles comptés (`LoadCopy`, `Register`, `Replace`), détacher les entités détruites, puis supprimer `Load`, `AddAsset`, `GetAsset`, `Unload` et les catégories (ADR-0037) | Approuvé et exécuté le 2026-09-21 (mode AUTO) : T0.1–T4.1, T7.1–T9.1 ✅ ; T3.1, T3.2, T4.2, T4.3, T5.1, T6.1 🧪 (vérifications visuelles de l'auteur : démos, éditeur, matériaux, environnements). `CasaEngine.Tests` 1780/1781 (seul échec préexistant). Reste : recette Alundra M2 et vérificateur final. Branche empilée sur `chantier/asset-handles`. Consommateur : plan parent `docs/plan-migration-handles.md`. |
