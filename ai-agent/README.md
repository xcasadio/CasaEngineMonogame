...
| --- | --- | --- |
| [play-in-editor-tasks.md](tasks/play-in-editor-tasks.md) | Play-in-Editor (Play/Pause/Stop dans le viewport + scripts compilés/rechargés à la volée) | Terminé sur la branche `play-in-editor` : toutes les tâches ✅, validations exécutées via l'automation `--play-smoke` (SampleProject + RPGDemo, 2026-08-18). Reste humain : ressenti clavier/manette en jeu. |
...
| [audio-system-tasks.md](tasks/audio-system-tasks.md) | Moteur de son V1 : bus de mixage, asset `.sound`, SFX one-shot/loop, streaming WAV des musiques, éditeur (branche `audio-system`) | 26 tâches ✅, 4 en 🧪 (validation à l'oreille de la démo, et click-through éditeur : Create Sound, inspecteur `.sound`). Reste : merge de la branche. Analyse et décisions : [analysis-audio-system.md](audits/analysis-audio-system.md), doc : [audio-system.md](../docs/engine/audio-system.md). |
