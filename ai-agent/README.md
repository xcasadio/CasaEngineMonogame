...
| --- | --- | --- |
| [bound-screens-tasks.md](tasks/bound-screens-tasks.md) | Programme « écrans liés » : les écrans d'un jeu deviennent des assets du projet liés à des view-models observables ; images nommées par asset (sprites et animations 2D) résolues par l'hôte ; binding MGUI sans allocation ; aller-retour sans perte et aperçu avec données de conception dans l'éditeur ; écran de dialogue remplaçable (ADR-0038, MGUI ADR-0016, parent ADR-0002) | Approuvé le 2026-09-24 (mode AUTO) ; phases 1 à 7 faites. **Phase 8 en cours** (réponses de l'auteur D13 à D18) : T6.2 dialogue incomplet, T4.4 « Save » enregistre les écrans, T4.5 confirmation avant de perdre un écran modifié, T4.6 Ctrl+S, puis B6 (parent). Reste en 🧪 : T2.3 (vérification visuelle du sample « Bound Images » de MGUI.Samples), T4.3 (vérification visuelle du sélecteur d'image de l'inspecteur), T5.1 (enregistrement d'un écran depuis l'éditeur, question O6). Points à arbitrer par l'auteur : O3, O5, O6 ; ordre des merges (O2). Consommateur : plan parent `docs/plan-bound-screens.md` (B1–B5). |
