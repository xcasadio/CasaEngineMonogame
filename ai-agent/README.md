...
| [tilemap-depth-settings-tasks.md](tasks/tilemap-depth-settings-tasks.md) | Faire agir `TileMapDepthSettings` au rendu (étapes 4 et 5 de `docs/engine/tilemaps-gestion-profondeur.md`) | Reconnaissance faite le 2026-09-07 (5 surfaces, 3 relecteurs adverses, **3 réfutations retenues**). `ShouldRenderTiles` est le seul champ lu au rendu, à 3 sites ; onze propriétés `depth.*` sont analysées, testées et mortes. Plan **en attente d'arbitrage** sur P1 à P5 — la question ouverte est comment les couches chunkées participent à l'ordre global, sachant qu'elles n'entrent jamais dans la file triée et qu'aucun code ne convertit une clé de tri en Z. Additif pour tout le contenu sauf `RPGDemo/Maps/map_1_1.tileMap`, seule carte du dépôt qui exerce ces réglages. |
