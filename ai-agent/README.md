...
| --- | --- | --- |
| [xaml-runtime-bridge-tasks.md](tasks/xaml-runtime-bridge-tasks.md) | Pont XAML runtime : charger les écrans MGUI depuis un `.xaml`, puis y migrer les dix écrans construits en C# | Plan rédigé le 2026-09-20, deux relectures adverses, quatre blocages corrigés, **approuvé par l'auteur le 2026-09-20**, exécution en cours. Né d'une règle d'auteur : la structure des écrans se déclare en XAML, le code retrouve par nom et pousse les valeurs. Douze tâches en six phases. Tranché par l'auteur : l'enveloppe est `.uiscreen` et l'éditeur gagne la route de document qui lui manquait (D11, T0.4) ; les quatre `.screen` hérités — données mortes, aucun lecteur, déjà remplacées par les écrans C#, l'une vide — sont relues puis supprimées (D12, T0.5). Restent ouverts O2 (où vit le XAML d'un écran du moteur, pas d'un jeu), O4 (`ResourceFiles` n'a aucun consommateur : sémantique non inventée) et O6 (la constante `.screen` et sa route deviennent mortes : retrait laissé à l'auteur). |
