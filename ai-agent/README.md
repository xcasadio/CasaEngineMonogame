...
| [mgui-package-alignment-tasks.md](tasks/archive/mgui-package-alignment-tasks.md) | Alignement sur la mise à jour du sous-module MGUI : packages MonoGame et FontStashSharp, renommage des namespaces de brushes, packages de test, outils `dotnet-mgcb` | **Terminé le 2026-09-13**, archivé. Six tâches ✅ sur la branche `fix/mgui-package-versions` (non mergée, non poussée), verifier CONFIRMED. Deux points ouverts à trancher : O1 (Content Browser dessiné en blocs blancs) et O2 (test `EditorThemeAsset_Disables_Docking_Accent_Bars` en échec sur la partie `Accent` du docking MGUI). Dans MGUI, `develop` et `casaengine/pin-fontstashsharp` ne sont pas poussés : les pousser avant CasaEngine. |
