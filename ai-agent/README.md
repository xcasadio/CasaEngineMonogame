...
| --- | --- | --- |
| [mgui-message-boxes-tasks.md](tasks/mgui-message-boxes-tasks.md) | Les 13 boîtes de message natives de l'éditeur deviennent des boîtes MGUI : contrôle `MGMessageBox` dans MGUI (surcouche modale du bureau), cause « fenêtre flottante fermée entière » dans `PanelClosing`, file de boîtes de l'éditeur, questions d'enregistrement asynchrones (fermer un écran, quitter, ouvrir un monde) ; sélecteurs de fichiers natifs gardés (ADR-0041, MGUI ADR-0018) | Approuvé et exécuté le 2026-09-25 (mode AUTO). T0.1, T1.2, T2.1, T2.2, T4.1 ✅. Vérificateurs frais **CONFIRMED** sur T1.2 + T3.1 puis sur tout le chantier, aucun constat P0–P2 (avis P4 reportés : O5–O9). `MGUI.Tests` 3083/3083, `CasaEngine.Tests` 1931/1931. Sortie automatisée avec un écran modifié vérifiée dans l'éditeur réel. Reste 🧪 pour l'auteur : T1.1 (page « MessageBox » du Compendium), T2.3 (suppressions, Properties, messages du lanceur), T3.1 (chaque geste de fermeture d'un écran modifié), T3.2 (File > Exit et croix de la fenêtre, chaque réponse), T3.3 (ouvrir un monde avec le monde courant modifié). O4 corrigé et fusionné dans `develop` de MGUI (`c831b53`). **Chantier fusionné** le 2026-09-25 à la demande de l'auteur : MGUI dans `develop` (`7a801e8`, ADR renumérotée 0041), moteur dans `main` (`60635e0e`, après intégration de `main` et du chantier audio : les deux solutions sans erreur, `CasaEngine.Tests` 1957/1957). Push de `develop` (MGUI) et de `main` (moteur) à faire par l'auteur : le hook du dépôt refuse tout `git push`. |
| [audio-stereo-voices-mute-tasks.md](tasks/archive/audio-stereo-voices-mute-tasks.md) | Voix stéréo logicielles aux gains gauche/droite exacts, modifiables pendant la lecture, et son coupé porté par les réglages du projet (ADR-0039, ADR-0040) | Plan écrit le 2026-09-25, **Terminé le 2026-09-26**, archivé. T1–T3 ✅ en mode AUTO, vérificateur **CONFIRMED** ; `CasaEngine.Tests` 1914/1914 à la clôture. Validé en jeu par l'auteur le 2026-09-26 (recette T5.3 du plan parent : Launcher et éditeur). Reportés : O1 (P3, `GetVoiceStereoGains` répond encore après `Stop`) et O2 (P4). Consommateur : plan parent `docs/plan-audio-mix-exact-muet.md` (T1.1 à T1.4). |
| [bound-screens-tasks.md](tasks/bound-screens-tasks.md) | Programme « écrans liés » : les écrans d'un jeu deviennent des assets du projet liés à des view-models observables ; images nommées par asset (sprites et animations 2D) résolues par l'hôte ; binding MGUI sans allocation ; aller-retour sans perte et aperçu avec données de conception dans l'éditeur ; écran de dialogue remplaçable (ADR-0038, MGUI ADR-0016, parent ADR-0002) | Approuvé le 2026-09-24 (mode AUTO) ; phases 1 à 7 faites. **Phase 8 faite** (réponses de l'auteur D13 à D18), chaque tâche vérifiée CONFIRMED : T6.2 dialogue incomplet, T4.4 « Save » enregistre les écrans (prouvé sur les écrans d'Alundra par B6), T4.5 confirmation avant de perdre un écran modifié, T4.6 Ctrl+S. Vérifications visuelles de l'auteur faites le 2026-09-24 (T2.3, T4.3, T4.5, T4.6). T4.7 (le titre d'un onglet, dont le `*` d'un document modifié, suit son panneau) vérifiée CONFIRMED, en 🧪 pour le coup d'œil de l'auteur après une vraie saisie. Points à arbitrer par l'auteur : O3, O5 (O6 tranché par D13) ; merges MGUI develop, moteur main, parent main faits en local (D16), push par l'auteur. Consommateur : plan parent `docs/plan-bound-screens.md` (B1–B6). |
| [asset-handles-migration-tasks.md](tasks/asset-handles-migration-tasks.md) | Migrer tous les appelants vers les handles comptés (`LoadCopy`, `Register`, `Replace`), détacher les entités détruites, puis supprimer `Load`, `AddAsset`, `GetAsset`, `Unload` et les catégories (ADR-0037) | Approuvé et exécuté les 2026-09-21 et 2026-09-22 (mode AUTO) : T0.1–T4.1, T7.1–T9.1 ✅ ; recette Alundra 389 → 390 → 389 tenue (M2 du plan parent) ; vérificateur final **CONFIRMED** sans constat P0–P2 (une remarque P3 et deux P4 reportées, O3–O5). `CasaEngine.Tests` 1780/1781 (seul échec préexistant, corrigé sur `main`). Reste : T3.1, T3.2, T4.2, T4.3, T5.1, T6.1 🧪 (vérifications visuelles de l'auteur : démos, éditeur, matériaux, environnements), puis archivage. Branche empilée sur `chantier/asset-handles`. Consommateur : plan parent `docs/plan-migration-handles.md`. |
| [asset-handles-tasks.md](tasks/archive/asset-handles-tasks.md) | Ressources comptées avec libération différée, polices bitmap comme ressources et registre de polices du jeu (ADR-0036) | Plan écrit le 2026-09-21, **Terminé le 2026-09-21**, archivé. T0.1–T4.1 ✅ en mode AUTO, vérificateur de clôture **CONFIRMED** sans constat P0–P2 (quatre avis P4 reportés). `CasaEngine.Tests` 1738/1739, le seul échec étant préexistant et hors périmètre (docking de l'éditeur, signalé à part) ; `MGUI.Tests` 2989/2989. Recette Alundra D5.f passée : font3 chargée une seule fois et gardée à travers le changement de carte. Restent à l'auteur : le merge de `chantier/asset-handles` ici et dans MGUI, et la recette en jeu D6 du plan parent. Né d'un défaut trouvé en jeu par le portage Alundra (textes de l'inventaire hors de `font3` après un changement de carte) ; la recette visible est la tranche D5.f du plan parent. |
| [xaml-runtime-bridge-tasks.md](tasks/archive/xaml-runtime-bridge-tasks.md) | Pont XAML runtime : charger les écrans MGUI depuis un `.xaml`, puis y migrer les dix écrans construits en C# | **Terminé le 2026-09-20**, archivé. Treize tâches ✅/🧪, verifier de clôture **CONFIRMED** sans aucun constat P0–P2, suite 1687 / 1686 verts (+54, 0 régression). Né d'une règle d'auteur : la structure des écrans se déclare en XAML, le code retrouve par nom et pousse les valeurs. `grep "new MGWindow("` ne rend plus rien dans le runtime. ADR-0035. Tranché par l'auteur : l'enveloppe est `.uiscreen`, l'éditeur gagne sa route de document, et les quatre `.screen` hérités sont relevés puis supprimés. **Restent à valider à l'œil par l'auteur** (rien de cela ne passe par la suite) : double-clic sur un `.uiscreen` dans l'éditeur, chargement de RPGDemo sans avertissement de catalogue, les neuf écrans en jeu, et une vue de moins de 460 px de haut pour clore l'avis A1. Reste ouvert : O4 (`ResourceFiles` sans consommateur), O6 (la constante `.screen` devenue morte : rupture d'API, décision de l'auteur), O7 (harnais de tests dupliqués), et les trois manques de MGUI du [rapport dédié](audits/mgui-gaps-from-xaml-screens.md). |
| [screen-effect-above-ui-tasks.md](tasks/archive/screen-effect-above-ui-tasks.md) | Effet d'écran au-dessus de l'interface MGUI, puis canal alpha et voile sans test de profondeur | **Terminé le 2026-09-20**, archivé. Six tâches ✅, verifier de clôture CONFIRMED, smoke validé en jeu par l'auteur. ADR-0033 et ADR-0034. Reste ouvert, décision d'architecture de l'auteur : quelles phases de l'image un voile plein écran doit couvrir, la couche par défaut étant dépassée par les composants dessinés en phase 3 (environ 7 000 pixels mesurés). Deux avis à garder au bump du pointeur : le paramètre optionnel ajouté est compatible en source mais pas en binaire, et le voile ignore désormais la profondeur inconditionnellement, donc un fondu soustractif assombrit des sprites qu'il épargnait. |
...
| [ui-integration.md](tasks/ui-integration.md) | UI Root, screen stack, overlay/world, game states | Réconcilié avec le code le 2026-09-06 : 35 cases cochées avec preuve, 42 restent ouvertes (câblage per-view de `ViewMouseViewport`, World UI, game screen states, démo d'intégration). |
| [tilemap-depth-settings-tasks.md](tasks/archive/tilemap-depth-settings-tasks.md) | Faire agir `TileMapDepthSettings` au rendu (étapes 4 et 5 de `docs/engine/tilemaps-gestion-profondeur.md`) | **Terminé le 2026-09-07**, archivé. Quatre tâches ✅, moteur 1618 verts, portage 815 inchangé, verifier CONFIRMED sur les 484 `.tileMap` réels (bit-identique sans métadonnée). Étape 6 explicitement non faite ; moitié « erreur » de l'étape 1 reportée (D4). Quatre avis P4 dans la clôture, dont : une couche sans `name` n'avertit jamais. |
...
| [static-model-import-tasks.md](tasks/static-model-import-tasks.md) | Import de modèles 3D statiques | Task 6.2 : tester l'import via l'éditeur. Références périmées annotées le 2026-09-06 (projets WPF et importeurs Assimp supprimés, voir ADR-0019). |
...
| [mgui-package-alignment-tasks.md](tasks/archive/mgui-package-alignment-tasks.md) | Alignement sur la mise à jour du sous-module MGUI : packages MonoGame et FontStashSharp, renommage des namespaces de brushes, packages de test, outils `dotnet-mgcb` | **Terminé le 2026-09-13**, archivé ; mergé dans `main` le 2026-09-14 (non poussé), verifier CONFIRMED. O1 (Content Browser dessiné en blocs blancs) est corrigé le 2026-09-14 sur la branche `fix/ui-primitive-blend-state` : `CasaDrawStateController` passe désormais le blend state résolu à `PrimitiveBatch.Begin`, comme MGUI `df22af7`. Reste à trancher O2 (test `EditorThemeAsset_Disables_Docking_Accent_Bars` en échec sur la partie `Accent` du docking MGUI). Dans MGUI, `casaengine/pin-fontstashsharp` (`0f7eb16`, le commit que référence CasaEngine) est mergée dans `develop` (`23cab97`), non poussé : pousser `develop` de MGUI avant `main`. |
...
Plans terminés (ou explicitement archivés) : fichiers de guidage des agents IA ([ai-guidelines-tasks.md](tasks/archive/ai-guidelines-tasks.md), 2026-09-06 : `AGENTS.md` source unique, règles et agents jumeaux `.github`/`.claude`, skills, `settings.json` et hook, 32 ADR ; audit [analysis-ai-agent-files.md](audits/analysis-ai-agent-files.md)), nouvel éditeur MGUI, multi-view/ViewManager v2, drag & drop d'assets, content browser, panels contextuels, layouts par mode, import de modèles statiques (V1), materials (plans `[ARCHIVE]` remplacés par la modernisation), audits d'architecture et de hiérarchie exécutés, coroutines V1, particules, animation moderne, timeline générique, profondeur TileMap, sprite viewer/thumbnails, portage skeletal blending, LightComponent, forward lighting/shadows, navigation V1, message bus World, nettoyage `Save(JObject)`, backlog V1 du screen editor, alignement sur la mise à jour du sous-module MGUI (2026-09-13).
