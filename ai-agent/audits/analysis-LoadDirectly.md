...
C'est cohérent avec le constat déjà posé dans [analysis-example-project.md](analysis-example-project.md) sur les fuites GPU des vieilles démos (`Clean()` vides). C'est objectivement le pire des quatre mécanismes du point de vue architecture, alors qu'il n'est même pas marqué `[Obsolete]` — il est donc invisible dans les warnings de build.
