...
4. **Réconcilier les deux flags d'input.** Garder un seul chemin (proposition : `PlayerController.IsInputEnable` comme source, la façade du point 3 l'applique ; supprimer `Pawn.InputEnabled` ou le déprécier). Corrige au passage le bug latent de `ScriptSign`. **[Implémenté le 2026-08-19 — Pawn.InputEnabled supprimé, PlayerController.IsInputEnable est l'unique gate ; commit 8710ae09.]**
5. **Nettoyer le code mort** : `AIController` moteur (vide, doublonné par les briques AI réelles : navigation driver, steering bridge, FSM), back-référence `Pawn.Controller` jamais assignée, `Player` vide (fusionner avec `LocalPlayer` ou l'étoffer le jour où un besoin réseau existe). **[Implémenté le 2026-08-19 — back-ref Pawn.Controller et champs UE commentés supprimés ; AIController conservé (compat PlayerStartupSettings) et devenu porteur du mode AI ; Player conservé comme base future ; commits 8d5ce0c5, 8710ae09. La classe `Pawn` elle-même a ensuite été supprimée le 2026-08-19 (aucune empreinte de sérialisation ; commits bf3ac54f, 0c9c5598).]**
6. **Brancher le multi-joueur local de bout en bout** (optionnel, plus tard) : `InitializePlayerControllers` sait déjà presque tout faire pour N joueurs ; il manque une boucle sur les `PlayerStart` par index et la fin du `// TODO` sur `LocalPlayer`. **[Implémenté le 2026-08-19 côté spawn/contrôleurs — un PlayerController par index de PlayerStart (player_index) ; join/leave et appairage devices restants ; commit c3d4133d.]**
