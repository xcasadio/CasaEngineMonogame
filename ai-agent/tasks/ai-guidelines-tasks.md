...
- Note de validation (2026-09-06) : script `.claude/hooks/block-commit-on-main.ps1` (Windows PowerShell, lecture du JSON sur stdin, découpage en segments et tokens, résolution du dépôt visé par `-C`, `cd` ou `cwd`, `--git-dir`/`--work-tree` → non résolu) câblé dans `.claude/settings.json` (`PreToolUse`, matcher `Bash`, `powershell.exe -NoProfile -ExecutionPolicy Bypass -File`, forme documentée par la doc hooks). Tests unitaires sur un worktree temporaire de `main` créé dans le dossier de travail de la session (`git worktree list` vérifié avant : `main` extraite nulle part) : a) `git commit` cwd=worktree → deny ; b) `git -C <worktree> commit` → deny ; c) `cd <worktree> && git commit` → deny ; d) `git commit` sur `ai-guidelines` → aucune sortie ; e) `git status` → aucune sortie ; f) `git -C <worktree> push` → deny push ; g) `fd . -x git push` → deny push ; h) `git --git-dir=… commit` → deny non résolu ; i) `git commit -F - <<'EOF' … git push … EOF` sur `ai-guidelines` → aucune sortie (les corps de heredoc sont retirés avant l'analyse : le premier essai de commit de cette tâche avait été refusé parce que son message contenait « git push ») ; j) heredoc + `git -C <worktree> commit` → deny main. Les tests sont lancés depuis un script du dossier de travail de la session, car le hook, déjà actif, refusait la ligne de commande de test elle-même quand elle contenait ces mots. Test de bout en bout par l'outil Bash de la session, sans redémarrage : `git -C <worktree> commit --dry-run` **refusé par le hook** (message « Commit sur main interdit : cree une branche dediee (AGENTS.md, D3). » renvoyé à l'agent), `git commit --dry-run` sur `ai-guidelines` exécuté (« Changes not staged for commit »). Aucun commit effectué pendant les tests ; worktree supprimé (`git worktree remove` + `prune`) ; branche de l'arbre principal toujours `ai-guidelines`.
...
- Note de validation (2026-09-06) : cinq ADR (0028 règle de couches et découpage en assemblies différé, 0029 hiérarchie de dossiers et namespaces, 0030 AGENTS.md source unique et outillage, 0031 règles de travail des agents, 0032 langue et pratique ADR) rédigés par un rédacteur sur modèle sonnet (P4) et relus par la session principale : sources relues (`structure-analyze-tasks.md:31-40` : les deux formulations de la règle de couches sont citées verbatim ; `CasaEngine_layering_project_split_evaluation.md:13-17, 55-58` ; `CasaEngine_folder_hierarchy_namespace_compatibility.md:11-49`), fidèles ; faits vérifiés : un seul `CasaEngine.csproj`, dossiers `Core/Logging`, `Core/Math`, `Framework/Application`, `Framework/Scene/Entities`, `Framework/Configuration` présents, `Core/Parsing` et `Core/Threading` absents comme leurs anciens noms (consigné « not found, unverified » dans ADR-0029). Les tâches de refactor exécutées (inventaire architecture 25-35) restent exclues, sauf `IGameplayProxy.Clone()` rattaché à la règle de couches. Une phrase de l'ADR-0032 sur l'état de l'index a été actualisée avant intégration. Total du rétro-remplissage : **32 ADR** (0001 → 0032), index complet dans `docs/decisions/README.md`.
...
- Note de validation (2026-09-06) : chaque correction vérifiée dans le code avant édition. `Game1` → `GameEditor` (`CasaEngine.Editor/GameEditor.cs`, qui porte `OnEditorAssetSaved` et `GetOrCreateMaterialViewportPanel`, et instancie `CasaGameRenderHost<GameEditor>`) dans `material-hot-reload-flow.md`, `casaengine-mgui-backend.md`, `environment-system-v1.md`, `architecture-entry-points.md` ; chemins réels par `fd` : `Framework/Application/CasaEngineGame.cs`, `Framework/Scene/Entities/Components/StaticModelComponent.cs`, `Framework/Configuration/Project/ProjectSettingsHelper.cs`, `CasaEngine.Editor/Runtime/EditorViewContext.cs` ; hôtes WPF `EngineHost.cs`/`ViewportControl.cs` absents → remplacés par une note et `WorldViewportPanel` ; `plan-e3-collisions.md` introuvable partout → « plan E3 collisions, non conservé dans le dépôt » (deux docs) ; `MGUI/docs/graph_node_system_agent_plan.md` absent → `MGUI/docs/Tasks/graph-tasks.md` (existant) avec mention de l'ancien nom ; `ai-agent/material-preview-smoke.txt` → `editor-material-preview-smoke.txt` à la racine (capture du 2026-04-05, contient bien des lignes `Override`) ; `CasaEngine.AISamples` et samples `westworld` absents de tout `.csproj`/`.cs` → commandes marquées historiques ; `D-E12-5` défini nulle part → identifiant retiré du titre ; `GameMode.cs` supprimé (remplacé par `GameplayMode.cs`, `RPGActionGameMode : GameplayMode`) → section « Constats vérifiés » de `gameplay-mode.md` requalifiée en constat d'origine avec lien vers ADR-0024. Mentions historiques de BulletSharp conservées au passé dans `collision-2d-3d-architecture.md` et `gameplay-csproj-scaffolding.md` (comparaisons datées). Hors périmètre, signalé en O5 : un commentaire de code cite encore `CasaGameRenderHost<Game1>` (`CasaEngine.Editor/GameEditor.cs:264`).
...
- Note de validation (2026-09-06), une correction par constat avec le fichier de code qui l'établit : `[dec-22]` `CompiledMaterial` est bien lu par le draw path (`Rendering/Draw/RenderItem.cs:31,53,77,82`, `RenderStateCache.cs:23,44`, `ShadowPass.cs:106`, alimenté par `StaticMeshRendererComponent.cs:276-320,457` depuis la `MaterialBase`) : la liste des consommateurs de `materials-sources-of-truth.md` complétée et le « gap » daté, `materials-workflow.md` précisé (binding par `MaterialBase`) ; `[dec-23]` aucun `BasicEffect` dans le runtime, le renderer de particules charge `BuiltInShaderCatalog.TexturedPrimitiveContentName` (`Framework/Particles/Rendering`, `_effect = Game.Content.Load<Effect>(...)`) : `particle-system-v1-v2-migration.md` corrigé ; `[dec-24]` `Camera3dIn2dAxisComponent` absent du code, `rendering-2d-3d-spaces.md` déjà exact, l'audit en désaccord est en lecture seule : aucune modification ; `[dec-25]` `CasaEngine/Framework/AI/Navigation/NavigationAgentComponent.cs` existe : ligne du tableau de `navigation-engine-features.md` requalifiée (constat d'origine, existe depuis) ; `[dec-26]` `CharacterControllerComponent` livré : constat d'origine requalifié dans `character-controller-features.md` ; `[dec-27]` `animation2d_editor_casaengine.md` : la phrase de requalification en tête nomme désormais les sections concernées (15, 16.1, 23.2, 28) et rappelle la primauté des décisions verrouillées, sans réécrire le corps.
...
- Note de validation (2026-09-06) : doc rédigée en anglais depuis le code par un rédacteur sur modèle sonnet (P4), 18 fichiers sources lus, relue par la session principale ; elle se termine par une « Source map » champ/comportement → `fichier:ligne` (43 entrées). Vérification par la session principale : les 21 noms de champs JSON cités (`animation_type`, `parts`, `tracks`, `events`, `collision_keyframes`, `default_*`, `*_keyframes`, `time_seconds`, `event_name`, `sprite_asset_id`, `target_part_id`, `event_track_name`, `lane_id`) sont tous présents dans `CasaEngine/Framework/Assets/Animations/`, `EditorAssetJsonSerializer.cs` ou `ObjectBase.cs` (`rg`), `lane_id` une seule fois côté lecture comme le décrit le doc ; lignes citées contrôlées par échantillon : `Animation2dTrackData.cs:18-21` (seul `Step`), `Animation2dCompositionSampler.cs:196-199` (`NotSupportedException`), `EditorAssetJsonSerializer.cs:243-267` (aucun `lane_id` écrit), `Animation2dData.cs:292` (inférence Loop depuis `restart`), `AssetLoaderRegistry.cs:41`, `Constants.cs:17` (`.anim2d`). Trois points laissés « unverified » dans le doc (absence d'un ancien schéma mono-sprite, caractère intentionnel de l'asymétrie `lane_id`, origine du fichier d'exemple). Ajoutée à `docs/README.md` ; la ligne « Points d'attention » de `ai-agent/README.md` qui signalait le fichier manquant est retirée.
...
- Note de validation (2026-09-06) : réconciliation préparée par un exécutant sur modèle sonnet (P4) dans le dossier de travail, avec un rapport item par item, relue par la session principale (diff des deux fichiers : uniquement des cases cochées avec preuve `fichier:ligne`, des chemins corrigés et des annotations « périmé », aucune autre réécriture). `ui-integration.md` : 35 cases cochées, 42 ouvertes ; `Framework/GUI/` → `Framework/UI/` ; constat notable : `ViewMouseViewport` existe mais n'est instancié nulle part. `static-model-import-tasks.md` : 25 tâches livrées, Task 6.2 restante ; `RiggedModelLoader`/`StaticModelImporter` supprimés par la migration glTF (ADR-0019) ; le doute laissé par l'exécutant sur `Import3dFileOptionsWindow` (Task 3.2) est tranché par `git log -S` : la fenêtre a existé puis a été supprimée par le commit `dc2f4d25` « remove unused projects ». Aucun des deux plans n'est archivé (travail restant réel) ; note de réconciliation datée ajoutée en tête de chacun ; lignes du tableau de `ai-agent/README.md` mises à jour ; plus aucun type disparu cité sans annotation (`rg`).
...
| O5 | Commentaire de code périmé : `CasaEngine.Editor/GameEditor.cs:264` cite `CasaGameRenderHost<Game1>` alors que la classe est `GameEditor` (ligne 305). Modification de code C#, hors périmètre de ce chantier : à corriger dans une tâche séparée. | T5.2 |
