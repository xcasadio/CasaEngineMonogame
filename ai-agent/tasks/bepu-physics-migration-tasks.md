...
| 2026-08-23 | RPGDemo : `AccessViolationException` en cassant l'herbe à l'épée | Les `ContactRecord` du pas courant stockaient des `CollidableReference` ; `OnHit` (pendant `SendEvents`) retire et dispose des collidables (`TileMapComponent.RemoveCollisionObject`) ; `DrawContacts` / `LatestContactPointsFor` déréféençaient ensuite `Simulation.Bodies/Statics[handle mort]` → lecture de mémoire non managée libérée (crash non déterministe). Un handle libéré puis réutilisé la même frame pouvait aussi attribuer le contact au mauvais corps. | `ContactRecord` devient un instantané autosuffisant (backends résolus + `PositionA` capturés dans `ConfigureContactManifold` pendant que la paire est vivante) ; `GetPosition(CollidableReference)` supprimé ; consommateurs réécrits. Tests `BepuContactRecordLifetimeTests` (2) : le test host crashe sur le parent (AV, 4 fois sur 5), 5/5 verts après. 0 B/frame mesuré. | `66835cec` |
| 2026-08-23 | RPGDemo : `KeyNotFoundException '-1'` (`TileSetData.GetTileData`) en coupant l'herbe | Un handler `OnHit` (`RemoveTile`) reconstruit le chunk de tuiles — corps et `TileCollisionManager` remplacés — pendant le dispatch du lot ; une autre collision du même lot (second « coupeur », ex. la pierre de l'octopus) référence encore l'ancien manager dont la cellule est vide. Défauts sous-jacents : aucun cycle de vie du manager ; `TileMapComponent.RemoveCollisionObject` appelait `ClearCollisionDataFrom(this)` (le composant) au lieu du manager, donc la physique ne terminait jamais ces collisions. | `TileCollisionManager.IsAttached`/`Detach()`, `GetTileData()` → null si détaché/vide, `RemoveTile()` no-op ; `TileMapComponent` suit le manager de chaque corps et appelle `ClearCollisionDataFrom(manager)` ; `SendEvents` ignore les collisions terminées pendant le lot ; `ClearCollisionDataOf` réentrant (liste poolée) et sans double `OnHitEnded` ; scripts RPGDemo avec garde null. 5 tests `TileMapBreakableTileTests` (3 rouges avant, dont le crash exact). | `77124926`, `3023063f`, + garde |
