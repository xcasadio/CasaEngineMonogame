...
| O6 | **Découvert à T2.2, hors périmètre de ce chantier, RÉSOLU le 2026-09-19** : `TileMapDemo` plantait sur une `NullReferenceException` (`PlayerComponent.cs:76`) dès qu'on l'exécutait, parce qu'`AnimatedSpriteComponent.InitializeWithWorld` vidait `Animations` et le reconstruisait uniquement depuis `_animationAssetIds`, jamais rempli par `AddAnimation(Animation2d)`, que `TileMapDemo.Initialize` utilise pourtant pour peupler ses animations. Corrigé à la cause par l'auteur hors de ce chantier, branche `chantier/fix-animated-sprite-code-animations` créée depuis `main` (commit `ac56aab0`, fusionnée ici) : les animations confiées à `AddAnimation` sont mémorisées et réenregistrées par la reconstruction, et `CurrentAnimation` est abandonnée avec les animations qu'elle désignait. Contournement retiré de `TileMapDemo.cs` : `entity.AddComponent(new PlayerComponent())` est de nouveau créé, et le personnage s'anime. | T2.2, résolu |
