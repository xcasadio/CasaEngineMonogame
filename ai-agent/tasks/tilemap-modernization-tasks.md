...
Ce plan transforme l'analyse de `ai-agent/audits/analysis_tilemaps_casaengine.md` en feuille de route actionnable pour un agent IA.
...
- 🧪 Ajouter une validation smoke testable : double-clic ou ouverture automatisée d'un `.tileMap` doit créer un onglet document et synchroniser Hierarchy/Inspector. Le snapshot diagnostics du document TileMap est branché ; le smoke `--open-asset Maps/map_1_1.tileMap` reste à relancer dans un contexte UI qui rend la main.
