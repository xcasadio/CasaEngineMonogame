...
Cadrage (correctif) : la première version cadrait la scène avec `ArcBallCameraComponent.SetCamera(position, target, up)` et la map apparaissait décalée en haut à gauche, mur invisible. Cause : `SetCamera` ne respecte pas son contrat — il stocke `Target = -target` et `Distance = -|position - target|`, ce qui, combiné à `ComputeViewMatrix` (`position = Target - Direction * Distance`), place la caméra en miroir sur Z dès que la cible n'est pas l'origine (voir le point « défaut préexistant de `SetCamera` » du Suivi). Défaut **préexistant** et **hors périmètre** : le corriger changerait le cadrage de la quinzaine d'appelants existants (démos et previews de l'éditeur), qui vivent avec parce qu'ils passent tous une cible nulle. La démo passe donc désormais par les **paramètres d'orbite explicites** (`Target` / `Yaw` / `Pitch` / `Distance`), dont la sémantique est sans ambiguïté, et le sol comme le mur sont recentrés sur l'origine (sol `LocalPosition = (-W/2, 0, -H/2)`, mur `(-W/2, H, W/2)`). Valeurs retenues : `Target = (0, MapHeight/2, 0)`, `Yaw = PI - 0.6`, `Pitch = 0.5`, `Distance = -70`. **Signe de `Distance` (2e correctif, remonté par l'utilisateur : commandes caméra inversées)** : la première version utilisait `Distance = +70`, alors que tout le reste du moteur stocke une distance **négative** (via `SetCamera`). Comme `position = Target - Direction * Distance`, inverser ce signe inverse `offset.Y`, donc le sens de `OrbitUp` — l'orbite verticale partait dans le mauvais sens par rapport aux autres démos (l'azimut, lui, est insensible au signe : `OrbitRight(+0.1)` donne +0.1 dans les deux cas). Transformation appliquée à cadrage constant : `Yaw' = Yaw + PI`, `Pitch' = -Pitch`, `Distance' = -Distance`. Mesuré : `OrbitUp(+0.1)` fait descendre la caméra de 6.30 unités, même sens qu'une caméra configurée par `SetCamera` (-1.56), contre +5.97 (montée) avant correction ; position de la caméra inchangée à 3.8e-6 près. Vérifié numériquement (sonde jetable, 1920 × 1080) : position réelle de la caméra `(34.686, 42.360, 50.701)`, `dot(normale, centre→caméra)` = 0.568 pour le sol et 0.694 pour le mur (donc aucun back-face culling), `BoundingFrustum.Contains` = `Contains` pour les deux maps, et les 8 coins de chaque map se projettent dans `[0,1920] × [0,1080]`. Statut 🧪 conservé : revalidation visuelle utilisateur nécessaire.
...
Cadrage (correctif) : la première version cadrait la scène avec `camera.SetPositionAndTarget(...)`, qui délègue à `ArcBallCameraComponent.SetCamera` — on ne voyait plus que le sol, l'écran d'arcade était hors champ. Même cause qu'en C3 : `SetCamera` stocke `Target = -target` et `Distance = -|position - target|`, ce qui place la caméra en miroir sur Z dès que la cible n'est pas l'origine (voir le point « défaut préexistant de `SetCamera` » du Suivi). Défaut **préexistant** et **hors périmètre** — le corriger changerait le cadrage de tous les appelants existants. La démo passe donc par les **paramètres d'orbite explicites** : `Target = ScreenCenter = (0, 2.65, 0)`, `Yaw = PI`, `Pitch = 0.22`, `Distance = -13` (signe de `Distance` négatif comme partout ailleurs dans le moteur, sinon l'orbite verticale est inversee — voir la note de C3). Vérifié numériquement (sonde jetable, 1920 × 1080) : position réelle de la caméra `(0, 5.487, 12.687)`, écran `dot(normale, centre→caméra)` = 0.976 et `BoundingFrustum.Contains` = `Contains` (coins projetés dans x ∈ [496, 1425], y ∈ [374, 698]), sol `dot` = 0.427, centre projeté en `(960, 833)` px (le sol de 24 × 24 déborde du frustum, d'où `Intersects` — attendu). Statut 🧪 conservé : revalidation visuelle utilisateur nécessaire.
